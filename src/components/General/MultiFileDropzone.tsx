...
const MultiFileDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div>
        <div className='active:shadow-none"  flex  flex-col gap-2 rounded-2xl border-2 border-dashed border-black bg-white px-6 py-3 font-semibold uppercase text-black transition-all duration-300 hover:translate-x-[-4px] hover:translate-y-[-4px] hover:rounded-md hover:shadow-[4px_4px_0px_black] active:translate-x-[0px] active:translate-y-[0px] active:rounded-2xl'>
          <div>
            {/* Main File Input */}
            <div
              {...getRootProps({
            ...
            >
              <div className='flex items-center gap-2 text-gray-500 dark:text-black'>
                ...
                <div className='flex w-12 justify-end text-xs'>
                  {progress === 'PENDING' ? (
                    <button
                      className='rounded-md bg-black p-1 transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-700'
                      onClick={() => {
                        void onChange?.(
